"""
Overwrite library_data/data/genres_example.json with current genres from the DB.

Safety: if the DB genre count is significantly lower than the existing example
file count, exit with status 1 (avoids accidentally wiping the seed dataset).

Run from the workspace root:
    python scripts/export_genres_example.py
    python scripts/export_genres_example.py --dry-run
    python scripts/export_genres_example.py --min-ratio 0.8
"""
...
EXAMPLE_PATH = (
    Path(__file__).resolve().parent.parent / "library_data" / "data" / "genres_example.json"
)
...
def _genres_to_seed_dict(genres_data: GenresData) -> dict:
    seed = {}
    for genre in genres_data.get_all_genres():
        seed[genre.name] = genre.to_json()
    return seed
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        metavar="RATIO",
        help=(
            "Abort if DB genre count / example genre count is below this ratio "
    ...
    seed = _genres_to_seed_dict(genres_data)
    db_count = len(seed)
    db_names = set(seed.keys())
    new_names = sorted(db_names - example_names)
    removed_names = sorted(example_names - db_names)

    print(f"Example genres ({args.output}): {example_count}")
    print(f"DB genres: {db_count}")
    print(f"New genres: {len(new_names)}")
    for name in new_names[:10]:
        print(f"  {name}")
    if len(new_names) > 10:
        print(f"  ... and {len(new_names) - 10} more")
    print(f"Removed genres: {len(removed_names)}")
    ...
    if example_count > 0:
        ...
        if ratio < args.min_ratio:
            print(
                f"error: DB genre count ({db_count}) is significantly lower than "
    ...
    if args.dry_run:
        print(f"Dry run — would write {db_count} genres to {args.output}")
        return 0
    ...
    print(f"Wrote {db_count} genres to {args.output}")
    return 0
