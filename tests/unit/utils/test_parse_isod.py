...
Regression coverage for a media API quirk: the reported duration
for a live/premiere broadcast with no determined length yet is given as
...
class TestParseIsod:
    ...
    def test_bare_p_with_no_unit_returns_zero(self):
        # The exact value observed from the media API for an
